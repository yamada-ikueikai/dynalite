...
module.exports = function updateItem(store, data, cb) {
  store.getTable(data.TableName, function (err, table) {
    ...
    if (
      (err = db.validateUpdates(data.AttributeUpdates, data._updates, table)) !=
      null
    )
      return cb(err);

    var itemDb = store.getItemDb(data.TableName),
      key = db.createKey(data.Key, table);
    ...
    itemDb.lock(key, function (release) {
      ...
      itemDb.get(key, function (err, oldItem) {
        ...
        var returnObj = {},
          ...
          paths = data._updates
            ? data._updates.paths
            : Object.keys(data.AttributeUpdates || {});
        ...
        err = data._updates
          ? db.applyUpdateExpression(data._updates.sections, table, item)
        ...
        returnObj.ConsumedCapacity = db.addConsumedCapacity(
          data,
          false,
          oldItem,
          item
    ...
    if (update.Action == "PUT" || update.Action == null) {
      ...
      if (update.Value.N) {
        ...
        var val =
          type == "L"
            ? update.Value[type]
            : update.Value[type].filter(function (a) {
...
function applyUpdateExpression(sections, table, item) {
  ...
  for (i = 0; i < sections.length; i++) {
    ...
    if (
      parent == null ||
      (typeof attr == "number" ? parent.L : parent.M) == null
    ) {
      return db.validationError(
    ...
    if (section.type == "remove") {
      ...
      if (alreadyExists) {
        existing[section.attrType] = existing[section.attrType].filter(
          function (val) {
            // eslint-disable-line no-loop-func
      ...
      if (section.attrType == "N") {
        ...
        existing[section.attrType] = existing[section.attrType].concat(
          section.val[section.attrType].filter(function (a) {
            // eslint-disable-line no-loop-func
...
function resolveValue(val, item) {
  if (Array.isArray(val)) {
    ...
    val = {
      N: new Big(val1.N)
        [val.type == "add" ? "plus" : "minus"](val2.N)
        .toFixed(),
    };
