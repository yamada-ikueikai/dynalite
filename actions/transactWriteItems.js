...
module.exports = function transactWriteItem(store, data, cb) {
  ...
  async.eachSeries(data.TransactItems, addActions, function (err) {
    if (err) {
      if (
        err.body &&
        (/Missing the key/.test(err.body.message) ||
          /Type mismatch for key/.test(err.body.message))
      )
    ...
    async.each(
      operationsbyTable,
      function ([tableName, ops], callback) {
  ...
  function addActions(transactItem, cb) {
    ...
    if (data.ReturnConsumedCapacity)
      options.ReturnConsumedCapacity = data.ReturnConsumedCapacity;
    ...
    if (transactItem.Put) {
      ...
      store.getTable(tableName, function (err, table) {
        if (err) return cb(err);
        if ((err = db.validateItem(transactItem.Put.Item, table)) != null)
          return cb(err);
        ...
        if (seenKeys[key]) {
          return cb(
            db.transactionCancelledException(
        ...
        itemDb.get(key, function (err, oldItem) {
          ...
          if ((err = db.checkConditional(transactItem.Put, oldItem)) != null)
            return cb(err);
      ...
      store.getTable(tableName, function (err, table) {
        if (err) return cb(err);
        if ((err = db.validateKey(transactItem.Delete.Key, table) != null))
          return cb(err);
        ...
        itemDb.lock(key, function (release) {
          ...
          itemDb.get(key, function (err, oldItem) {
            ...
            if (
              (err = db.checkConditional(transactItem.Delete, oldItem)) != null
            )
              return cb(err);
      ...
      store.getTable(tableName, function (err, table) {
        ...
        if (
          (err = db.validateUpdates(
            transactItem.Update.AttributeUpdates,
            transactItem.Update._updates,
            table
          )) != null
        )
          return cb(err);
        ...
        itemDb.lock(key, function (release) {
          ...
          itemDb.get(key, function (err, oldItem) {
            ...
            if (
              (err = db.checkConditional(transactItem.Update, oldItem)) != null
            )
              return cb(err);
            ...
            err = transactItem.Update._updates
              ? db.applyUpdateExpression(
                  transactItem.Update._updates.sections,
                  table,
                  item
                )
              : db.applyAttributeUpdates(
                  transactItem.Update.AttributeUpdates,
                  table,
                  item
                );
