...
var target = "TransactWriteItems",
  ...
  assertTransactionCanceled = helpers.assertTransactionCanceled.bind(
    null,
    target
  );
...
describe("transactWriteItem", function () {
  describe("serializations", function () {
    ...
    it("should return SerializationException when TransactItems.0.Delete.Key.Attr is not an attr struct", function (done) {
      this.timeout(60000);
      assertType(
        "TransactItems.0.Delete.Key.Attr",
        "AttrStruct<ValueStruct>",
        done
      );
    });
  ...
  describe("validations", function () {
    ...
    it("should return ValidationException for invalid metadata and missing requests", function (done) {
      assertValidation(
        {
          TransactItems: [],
          ReturnConsumedCapacity: "hi",
          ReturnItemCollectionMetrics: "hi",
        },
        [
          "Value 'hi' at 'returnConsumedCapacity' failed to satisfy constraint: " +
            "Member must satisfy enum value set: [INDEXES, TOTAL, NONE]",
    ...
    it("should return ValidationException when writing more than 25 items", function (done) {
      ...
      for (i = 0; i < 26; i++) {
        requests.push(
          i % 2
            ? { Delete: { Key: { a: { S: String(i) } } } }
            : { Put: { Item: { a: { S: String(i) } } } }
        );
      }
      assertValidation(
        { TransactItems: requests },
        [new RegExp("Member must have length less than or equal to 25")],
        done
      );
    });
    ...
    it("should return ResourceNotFoundException when fetching exactly 25 items and table does not exist", function (done) {
      ...
      for (i = 0; i < 25; i++) {
        requests.push(
          i % 2
            ? { Delete: { TableName: "a", Key: { a: { S: String(i) } } } }
            : {
                Put: {
                  TableName: "a",
                  Item: { a: { S: String(i) } },
          ...
          assertValidation(
            {
              TransactItems: [
                {
                  Put: { TableName: helpers.testHashTable, Item: { a: expr } },
                },
              ],
            },
    ...
    describe("functionality", function () {
      it("should write a single item", function (done) {
        ...
        batchReq.TransactItems = [
          { Put: { TableName: helpers.testHashTable, Item: item } },
        ];
      ...
      it("should put multiple items", function (done) {
        ...
        transactReq.TransactItems = [
          { Put: { TableName: helpers.testHashTable, Item: item } },
          { Put: { TableName: helpers.testHashTable, Item: item2 } },
        ];
      ...
      it("should write to two different tables", function (done) {
        ...
        transactReq = { TransactItems: [] };
        transactReq.TransactItems = [
          { Put: { TableName: helpers.testHashTable, Item: hashItem } },
          { Put: { TableName: helpers.testRangeTable, Item: rangeItem } },
        ];
        request(opts(transactReq), function (err, res) {
          if (err) return done(err);
          res.statusCode.should.equal(200);
          ...
          request(
            ...
            function (err, res) {
              ...
              request(
                helpers.opts("GetItem", {
                  TableName: helpers.testRangeTable,
                  Key: { a: rangeItem.a, b: rangeItem.b },
                  ConsistentRead: true,
                }),
                function (err, res) {
                  if (err) return done(err);
      ...
      it("should return ConsumedCapacity from each specified table when putting and deleting small item", function (done) {
        var a = helpers.randomString(),
          b = new Array(1010 - a.length).join("b"),
          item = {
            a: { S: a },
            b: { S: b },
            c: { N: "12.3456" },
          ...
          key2 = helpers.randomString(),
          key3 = helpers.randomNumber(),
        ...
        request(opts(batchReq), function (err, res) {
          if (err) return done(err);
          res.statusCode.should.equal(200);
          res.body.ConsumedCapacity.should.containEql({
            CapacityUnits: 4,
            TableName: helpers.testHashTable,
          });
          res.body.ConsumedCapacity.should.containEql({
          ...
          request(opts(batchReq), function (err, res) {
            ...
            res.body.ConsumedCapacity.should.containEql({
              CapacityUnits: 2,
              Table: { CapacityUnits: 2 },
              TableName: helpers.testHashNTable,
            });
            batchReq.ReturnConsumedCapacity = "TOTAL";
            batchReq.TransactItems[helpers.testHashTable] = [
              { DeleteRequest: { Key: { a: item.a } } },
              { DeleteRequest: { Key: { a: { S: key2 } } } },
            ];
            batchReq.TransactItems[helpers.testHashNTable] = [
              { DeleteRequest: { Key: { a: { N: key3 } } } },
            ];
            request(opts(batchReq), function (err, res) {
              if (err) return done(err);
              res.statusCode.should.equal(200);
      ...
      it("should return ConsumedCapacity from each specified table when putting and deleting larger item", function (done) {
        var a = helpers.randomString(),
          b = new Array(1012 - a.length).join("b"),
          item = {
            a: { S: a },
            b: { S: b },
            c: { N: "12.3456" },
          ...
          request(
            ...
            function (err, res) {
              ...
              res.body.Items.should.eql([
                { a: key.a, b: key.b, c: key.a, d: key.b, e: key.a, f: key.b },
              ]);
              request(
                helpers.opts("Query", {
                  TableName: helpers.testRangeTable,
                  ...
                  KeyConditions: {
                    a: {
                      ...
                      AttributeValueList: [key.a],
                    },
                    d: {
                      ComparisonOperator: "EQ",
                      AttributeValueList: [key.b],
                    },
                  },
                }),
                function (err, res) {
                  ...
                  res.statusCode.should.equal(200);
                  res.body.Items.should.eql([
                    { a: key.a, b: key.b, c: key.a, d: key.b },
                  ]);
