  ...
  if (update.Action == 'PUT' || update.Action == null) {
    ...
    if (update.Value.N) {
      if (item[attr] && !item[attr].N)
        return validationError('Type mismatch for attribute to update')
...
for (var i = 0; i < sections.length; i++) {
  ...
  if (section.type == 'set') {
    ...
    if (typeof section.val == 'string') {
      return validationError(section.val)
    }
  }
}
for (i = 0; i < sections.length; i++) {
  section = sections[i]
  var parent = mapPath(section.path.slice(0, -1), item)
  var attr = section.path[section.path.length - 1]
  if (parent == null || (typeof attr == 'number' ? parent.L : parent.M) == null) {
    return validationError('The document path provided in the update expression is invalid for update')
  }
  ...
  if (section.type == 'remove') {
    ...
    if (alreadyExists && Object.keys(existing)[0] != section.attrType) {
      return validationError('An operand in the update expression has an incorrect data type')
    }
    if (alreadyExists) {
    ...
    if (section.path.length == 1) {
      var err = traverseIndexes(table, function(attr, type) {
        if (section.path[0] == attr && section.val[type] == null) {
          return validationError('The update expression attempted to update the secondary index key to unsupported type')
        }
      })
      if (err) return err
    }
...
if (Array.isArray(val)) {
  ...
  val = mapPath(val.args[0], item) || resolveValue(val.args[1], item)
